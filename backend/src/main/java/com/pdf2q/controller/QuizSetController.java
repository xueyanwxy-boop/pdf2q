...
/**
 * 题库相关 HTTP 接口。
 * <p>需 {@code X-Owner-Token}；若带 {@code Authorization: Bearer} 则按云端账号读写。
 */
@RestController
...
public class QuizSetController {
  ...
  public List<QuizSetSummary> list(HttpServletRequest request) {
    return quizSetService.list(ownerTokenResolver.require(request), userId(request));
  }
  ...
  public QuizSetDetail create(HttpServletRequest request, @Valid @RequestBody CreateQuizSetRequest body) {
    return quizSetService.create(ownerTokenResolver.require(request), userId(request), body);
  }

  /** 题库详情（题目 + 当前进度）。GET /api/quiz-sets/{id} */
  @GetMapping("/{id}")
  public QuizSetDetail get(HttpServletRequest request, @PathVariable Long id) {
    return quizSetService.get(ownerTokenResolver.require(request), userId(request), id);
  }

  /** 保存答题进度。PUT /api/quiz-sets/{id}/progress */
  @PutMapping("/{id}/progress")
  public ProgressDto saveProgress(
      HttpServletRequest request,
      @PathVariable Long id,
      @Valid @RequestBody SaveProgressRequest body) {
    return quizSetService.saveProgress(
        ownerTokenResolver.require(request), userId(request), id, body);
  }

  /** 清空进度。POST /api/quiz-sets/{id}/progress/reset */
  @PostMapping("/{id}/progress/reset")
  public ProgressDto resetProgress(HttpServletRequest request, @PathVariable Long id) {
    return quizSetService.resetProgress(ownerTokenResolver.require(request), userId(request), id);
  }

  /** 删除题库及其进度。DELETE /api/quiz-sets/{id} */
  @DeleteMapping("/{id}")
  public Map<String, Object> delete(HttpServletRequest request, @PathVariable Long id) {
    quizSetService.delete(ownerTokenResolver.require(request), userId(request), id);
    return Map.of("ok", true);
  }

  private Long userId(HttpServletRequest request) {
    User user = authUserResolver.optional(request);
